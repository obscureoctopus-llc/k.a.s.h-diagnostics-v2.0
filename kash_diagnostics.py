...
class DiagnosticEngine:
    db_path: Path = Path("database.json")
    vehicle_db: Dict[str, Any] = field(default_factory=dict)

    def __post_init__(self) -> None:
        self.load_database()

    def load_database(self) -> None:
        """Loads universal taxonomy maps from json store."""
        try:
            ...
            log.error(f"Failed to parse diagnostic database: {e}")
...
# Target the exact physical GPIO serial pins assigned to your RS-485 rev2.1 expansion layout
GPIO_PORT = '/dev/ttyAMA0'
BAUD_RATE = 9600

class DiagnosticBridge:
    def __init__(self):
        self.latest_data = {"status": "INITIALIZING", "raw_frame": ""}
        try:
            self.ser = serial.Serial(GPIO_PORT, BAUD_RATE, timeout=1)
            print("🚀 K.A.S.H. HARDWARE LINKED: Listening on GPIO /dev/ttyAMA0")
        except Exception as e:
            print(f"⚠️ HARDWARE FAILURE: Unable to bind to GPIO pins. ({e})")
            self.ser = None
            ...
            if self.ser and self.ser.is_open:
                try:
                    if self.ser.in_waiting > 0:
                        # Read incoming vehicle line from RS-485 layer
                        raw_line = self.ser.readline().decode('utf-8', errors='ignore').strip()
                        print(f"📥 RX RS-485: {raw_line}")
                        
                        # Store the valid telemetry frame
                        self.latest_data = {
                            "status": "KASH READY",
                            "timestamp": time.time(),
                            "raw_frame": raw_line
...
# Spin up a small HTTP JSON server endpoint for your HTML dashboard to call
class DashServer(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == '/api/live':
            ...
            self.end_headers()
            self.wfile.write(json.dumps(bridge.latest_data).encode('utf-8'))
        else:
...
if __name__ == "__main__":
    ...
    # Run the hardware listener loop as a background thread
    hw_thread = threading.Thread(target=bridge.read_loop, daemon=True)
    hw_thread.start()
    
    # Serve JSON payloads on port 5000 for the frontend
